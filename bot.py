...
@bot.tree.command(
    name="consolidate",
    description="Automatically detect and merge similar channels"
)
async def consolidate(interaction: discord.Interaction):
    """Automatically find and merge similar channels."""
    ...
    if not similar:
        embed = discord.Embed(
            ...
            description="All channels have unique names. Nothing to consolidate.",
            color=discord.Color.green()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
        return
    
    # Automatically merge all similar channels
    merged_count = 0
    failed_count = 0
    merge_results = []
    
    try:
        for ch1, ch2, similarity in similar:
            similarity_percent = int(similarity * 100)
            try:
                # Keep the channel with the better name, delete the other
                # Use the first channel as the target
                await ch2.delete(reason=f"Consolidated with #{ch1.name} ({similarity_percent}% similar)")
                merged_count += 1
                merge_results.append(f"✅ **#{ch2.name}** → merged into **#{ch1.name}** ({similarity_percent}% match)")
                logger.info(f"Merged {ch2.name} into {ch1.name}")
                
            except Exception as e:
                failed_count += 1
                merge_results.append(f"❌ Failed to merge **#{ch2.name}**: {str(e)}")
                logger.error(f"Failed to merge {ch2.name}: {str(e)}")
        
        # Show results
        embed = discord.Embed(
            title="🔀 Channel Consolidation Complete",
            description=f"Processed {merged_count + failed_count} similar channel pairs",
        ...
        if len(results_text) > 1024:
            ...
            for i in range(0, len(merge_results), 3):
                chunk = merge_results[i:i+3]
                embed.add_field(
                    name=f"Results (Part {i//3 + 1})",
                    value="\n".join(chunk),
                    inline=False
                )
        else:
        ...
        embed.add_field(
            name="Summary",
            value=f"✅ **{merged_count}** channels merged\n"
                  f"❌ **{failed_count}** merges failed",
            inline=False
        )
        ...
        embed = discord.Embed(
            title="❌ Permission Denied",
            description="Bot doesn't have permission to delete channels.",
            color=discord.Color.red()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
    except Exception as e:
