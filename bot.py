...
async def help_command(interaction: discord.Interaction):
    """Display all active bot commands with real descriptions from the bot tree."""
    ...
    if not commands_list:
        embed = discord.Embed(
            ...
            description="The bot has no active commands registered.",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    
    # Build the embed with all active commands
    embed = discord.Embed(
        title="🤖 Discord Server Setup Bot - Commands",
        description=f"Here's everything you can do with this bot! ({len(commands_list)} commands)",
        color=BOT_COLOR
    )
    
    command_text = ""
    for cmd in commands_list:
        # Get the actual command name and description
        cmd_name = cmd.name
        cmd_description = cmd.description or "No description provided"
        command_text += f"**/{cmd_name}** — {cmd_description}\n"
