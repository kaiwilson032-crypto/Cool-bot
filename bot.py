                ...
                "categories": [
                    {
                        "name": "General",
                        "channels": [
                            {"name": "announcements", "type": "text", "visible_to": ["Member"]},
                            {"name": "general", "type": "text", "visible_to": ["Member"]},
                            {"name": "rules", "type": "text", "visible_to": ["Member"]}
                        ]
                    },
                    {
                        "name": "Voice Channels",
                        "channels": [
                            {"name": "Lobby", "type": "voice", "visible_to": ["Member"]},
                            {"name": "Gaming", "type": "voice", "visible_to": ["Member"]},
                            {"name": "AFK", "type": "voice", "visible_to": ["Member"]}
                        ]
                    },
                    {
                        "name": "Staff",
                        "channels": [
                            {"name": "staff-chat", "type": "text", "visible_to": ["Moderator"]},
                            {"name": "logs", "type": "text", "visible_to": ["Moderator"]}
                        ]
                    }
                ]
            },
                ...
                "categories": [
                    {
                        "name": "Welcome",
                        "channels": [
                            {"name": "welcome", "type": "text", "visible_to": ["Member"]},
                            {"name": "rules", "type": "text", "visible_to": ["Member"]},
                            {"name": "introductions", "type": "text", "visible_to": ["Member"]}
                        ]
                    },
                    {
                        "name": "Discussion",
                        "channels": [
                            {"name": "general", "type": "text", "visible_to": ["Member"]},
                            {"name": "events", "type": "text", "visible_to": ["Member"]},
                            {"name": "feedback", "type": "text", "visible_to": ["Member"]}
                        ]
                    },
                    {
                        "name": "Moderation",
                        "channels": [
                            {"name": "mod-chat", "type": "text", "visible_to": ["Moderator"]},
                            {"name": "reports", "type": "text", "visible_to": ["Moderator"]}
                        ]
                    }
                ]
            },
                ...
                "categories": [
                    {
                        "name": "General",
                        "channels": [
                            {"name": "announcements", "type": "text", "visible_to": ["Team"]},
                            {"name": "general", "type": "text", "visible_to": ["Team"]},
                            {"name": "watercooler", "type": "text", "visible_to": ["Team"]}
                        ]
                    },
                    {
                        "name": "Projects",
                        "channels": [
                            {"name": "project-alpha", "type": "text", "visible_to": ["Team"]},
                            {"name": "project-beta", "type": "text", "visible_to": ["Team"]},
                            {"name": "meetings", "type": "voice", "visible_to": ["Team"]}
                        ]
                    },
                    {
                        "name": "Management",
                        "channels": [
                            {"name": "exec-chat", "type": "text", "visible_to": ["Manager"]},
                            {"name": "performance", "type": "text", "visible_to": ["Manager"]}
                        ]
                    }
                ]
            }
...
class SetupView(discord.ui.View):
    ...
    async def select_template(self, interaction: discord.Interaction) -> None:
        ...
        self.selected_template = select_value
    ...
    async def create_template(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if not interaction.guild.me.guild_permissions.administrator:
            await interaction.followup.send(
                "❌ Bot requires Administrator permission to create server structure.",
                ephemeral=True
            )
            return
        ...
        try:
            ...
            for role_data in self.template_data.get('roles', []):
                try:
                    ...
                    logger.error(f"Failed to create role {role_data.get('name')}: {e}")
                ...
                try:
                    ...
                    for channel_data in category_data.get('channels', []):
                        try:
                            ...
                            logger.error(f"Failed to create channel {channel_data.get('name')}: {e}")
            ...
            await interaction.followup.send(
                f"❌ An error occurred while creating the template: {str(e)}",
                ephemeral=True
            )
...
class ConfirmUndoView(discord.ui.View):
    ...
    async def confirm_undo(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if guild_id not in self.bot.last_setup:
            await interaction.followup.send(
                "❌ No previous setup found to undo.",
                ephemeral=True
            )
            return
        ...
        try:
            ...
            await interaction.followup.send(
                f"❌ An error occurred while undoing the setup: {str(e)}",
                ephemeral=True
            )
...
class SetupBot(commands.Cog):
    ...
    @discord.app_commands.command(
        name="delete",
        description="Remove all server channels and roles (DANGEROUS)"
    )
    async def delete(self, interaction: discord.Interaction) -> None:
        """Delete all server channels and roles with confirmation"""
        if not interaction.user.guild_permissions.administrator:
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="You need Administrator permissions to use this command.",
    ...
    @discord.app_commands.command(
        name="info",
        description="View bot information and available commands"
    )
    async def info(self, interaction: discord.Interaction) -> None:
        ...
        embed.add_field(
            ...
            value=len(self.template_manager.get_templates()),
            inline=True
        )
        embed.add_field(
            name="Servers Using Bot",
