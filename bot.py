...
# ============================================================================
# SMART NAMING & CONSOLIDATION UTILITIES
# ============================================================================

def sanitize_channel_name(name: str) -> str:
    """Convert a channel name to Discord format (lowercase, hyphens only)."""
    ...
    name = name.strip('-')
    return name[:100]  # Discord max is 100 chars

def calculate_name_similarity(name1: str, name2: str) -> float:
    """
    Calculate similarity between two channel names (0-1).
    Accounts for prefixes, suffixes, and common variations.
    """
    n1 = sanitize_channel_name(name1).lower()
    n2 = sanitize_channel_name(name2).lower()
    ...
    if n1 in n2 or n2 in n1:
        overlap = len(min(n1, n2))
        return overlap / max(len(n1), len(n2))
    
    # Levenshtein-like similarity
    common_chars = sum(1 for c in n1 if c in n2)
    return common_chars / max(len(n1), len(n2))

def detect_similar_channels(guild) -> List[Tuple[discord.TextChannel, discord.TextChannel, float]]:
    """
    Detect similar channel names that could be consolidated.
    Returns list of (channel1, channel2, similarity) tuples with similarity > 0.6
    """
    text_channels = [ch for ch in guild.channels if isinstance(ch, discord.TextChannel)]
    similar_pairs = []
    
    for i, ch1 in enumerate(text_channels):
        for ch2 in text_channels[i+1:]:
            similarity = calculate_name_similarity(ch1.name, ch2.name)
            if similarity > 0.6 and ch1.name != ch2.name:
                similar_pairs.append((ch1, ch2, similarity))
    
    # Sort by similarity (highest first)
    return sorted(similar_pairs, key=lambda x: x[2], reverse=True)

def apply_smart_naming(template_name: str) -> Dict[str, str]:
    """
    Generate smart naming suggestions based on template.
    Returns mapping of template channel name -> suggested clean name
    """
    naming_rules = {
        "Professional": {
    ...
    return naming_rules.get(template_name, {})
...
# ============================================================================
# CHANNEL CONSOLIDATION COMMAND
# ============================================================================

class ConsolidateView(discord.ui.View):
    """View for confirming channel consolidation."""
    
    def __init__(self, channel1: discord.TextChannel, channel2: discord.TextChannel, target_name: str):
        super().__init__()
    ...
    @discord.ui.button(label="Merge Channels", style=discord.ButtonStyle.danger, emoji="🔀")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.confirmed = True
        ...
        try:
            # Rename first channel to target
            await self.channel1.edit(name=self.target_name)
            
            # Delete second channel
            await self.channel2.delete(reason=f"Consolidated with #{self.target_name}")
            
            embed = discord.Embed(
                title="✅ Channels Merged!",
                description=f"**#{self.channel2.name}** → merged into **#{self.target_name}**",
                color=discord.Color.green()
            )
            embed.add_field(
                name="Result",
                value=f"Channel **#{self.channel2.name}** has been deleted.\nAll discussions continue in **#{self.target_name}**",
                inline=False
            )
            
            await interaction.followup.send(embed=embed, ephemeral=True)
            logger.info(f"Consolidated {self.channel2.name} into {self.target_name}")
            
        except discord.Forbidden:
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="Bot doesn't have permission to rename/delete channels.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
        except Exception as e:
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="❌ Cancelled",
            description="Channel consolidation cancelled.",
            color=discord.Color.red()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
...
@bot.tree.command(
    name="consolidate",
    description="Detect and merge similar channels"
)
async def consolidate(interaction: discord.Interaction):
    """Find similar channels and offer to merge them."""
    ...
    similar = detect_similar_channels(interaction.guild)
    
    if not similar:
        embed = discord.Embed(
            title="✅ No Similar Channels",
            description="All channels have unique names.",
            color=discord.Color.green()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
        return
    ...
    embed = discord.Embed(
        title="🔍 Similar Channels Detected",
        description="Found channels that could be consolidated:",
        color=BOT_COLOR
    )
    
    for i, (ch1, ch2, similarity) in enumerate(similar[:5]):
        similarity_percent = int(similarity * 100)
        embed.add_field(
            name=f"Match #{i+1} ({similarity_percent}% similar)",
            value=f"**#{ch1.name}** ↔ **#{ch2.name}**",
            inline=False
        )
    ...
    if len(similar) > 5:
        embed.add_field(
            name="More Matches",
            value=f"...and {len(similar) - 5} more similar channel pairs",
            inline=False
        )
    
    embed.set_footer(text="Use /rename to consolidate specific channels")
    await interaction.followup.send(embed=embed, ephemeral=True)
...
class RenameChannelSelect(discord.ui.Select):
    """Select channel to rename."""
    
    def __init__(self, channels: List[discord.TextChannel]):
        options = [
            discord.SelectOption(label=ch.name[:100], value=str(ch.id), emoji="📝")
            for ch in channels[:25]
        ]
        super().__init__(placeholder="Select channel to rename...", options=options, min_values=1, max_values=1)
...
class RenameModal(discord.ui.Modal, title="Rename Channel"):
    ...
    new_name = discord.ui.TextInput(
        ...
        placeholder="Enter new name (lowercase, hyphens only)",
        min_length=1,
        max_length=100
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Sanitize the name
        sanitized = sanitize_channel_name(self.new_name.value)
        
        try:
            await self.channel.edit(name=sanitized)
            
            embed = discord.Embed(
                title="✅ Channel Renamed!",
                description=f"**#{self.channel.name}** → **#{sanitized}**",
                color=discord.Color.green()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
            logger.info(f"Renamed channel {self.channel.name} to {sanitized}")
            
        except discord.Forbidden:
            embed = discord.Embed(
                title="❌ Permission Denied",
                description="Bot doesn't have permission to rename this channel.",
                color=discord.Color.red()
            )
            await interaction.followup.send(embed=embed, ephemeral=True)
        except Exception as e:
...
class RenameView(discord.ui.View):
    """View for channel rename selection."""
    
    def __init__(self, channels: List[discord.TextChannel]):
        super().__init__()
        self.add_item(RenameChannelSelect(channels))

@bot.tree.command(
    name="rename",
    description="Rename channels with smart formatting"
)
async def rename(interaction: discord.Interaction):
    """Interactively rename channels with smart sanitization."""
    ...
    if not text_channels:
        embed = discord.Embed(
            title="ℹ️ No Text Channels",
            description="This server has no text channels.",
            color=discord.Color.blue()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    embed = discord.Embed(
        ...
        description="Select a channel to rename. Names will be auto-formatted (lowercase, hyphens only).",
        color=BOT_COLOR
    )
    
    view = RenameView(text_channels)
    await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
...
async def help_command(interaction: discord.Interaction):
    """Display all bot commands with descriptions."""
    
    commands_info = {
        "Setup Commands": [
            ("/setup", "Apply a pre-built template to your server (Professional, Community, Gaming)"),
            ("/delete", "Delete all channels except the current one"),
        ],
        "Channel Management": [
            ("/rename", "Rename channels with smart formatting (auto-sanitizes names)"),
            ("/consolidate", "Detect and merge similar channels to reduce clutter"),
            ("/reorder", "Organize categories by priority (1-5)"),
        ],
        "Fun Commands": [
            ("/goose", "Get a random goose image in a beautiful embed"),
        ],
        "Utility": [
    ...
    embed = discord.Embed(
        title="🤖 Discord Server Setup Bot - Commands",
    ...
    embed.add_field(
        ...
        value="Most commands require **Administrator** permissions\n"
              "The bot needs **Manage Channels**, **Manage Roles**, and **Manage Guild** permissions",
        inline=False
    )
    
    embed.add_field(
        name="💡 Tips",
        value="• Use `/setup` first to create a complete server structure\n"
              "• Use `/consolidate` to find and merge similar channels\n"
              "• Channel names are automatically formatted (lowercase, hyphens only)\n"
              "• All commands show detailed status and confirmations",
        inline=False
    )
    
    embed.set_footer(
        text="React to any bot message to get more information"
    )
